...
O pipeline está configurado em [`.github/workflows/cypress.yml`](.github/workflows/cypress.yml) e utiliza a action oficial [`cypress-io/github-action@v6`](https://github.com/cypress-io/github-action).

### Quando é disparado automaticamente

| Evento | Condição |
|---|---|
| `push` | Commit enviado para `master` ou `main` |
| `pull_request` | PR aberto ou atualizado com destino a `master` ou `main` |
| `workflow_dispatch` | Execução manual pela interface do GitHub |

### Jobs

Os dois jobs rodam **em paralelo**, de forma independente:

```
Cypress Tests
├── Testes de API      →  npm run cy:api       (cypress/e2e/api/**)
└── Testes de Frontend →  npm run cy:frontend  (cypress/e2e/frontend/**)
```

Cada job executa os seguintes passos:

1. **Checkout** — clona o código no runner Ubuntu
2. **Instalar e executar** — `cypress-io/github-action` instala as dependências e roda os testes
3. **Upload do relatório** — salva `cypress/reports/` como artefato (sempre, mesmo com falhas)
4. **Upload de screenshots** — salva `cypress/screenshots/` como artefato (somente se falhar)

### Como disparar manualmente

1. Acesse a aba **Actions** do repositório:  
   `https://github.com/Fabio-Costa986/projeto-ambev-automacao/actions`

2. No menu lateral, clique em **Cypress Tests**

3. Clique no botão **Run workflow** → selecione a branch → **Run workflow**

### Como acessar os artefatos gerados

Após a conclusão de um run:

1. Clique no run desejado na aba **Actions**
2. Role até a seção **Artifacts** no final da página
3. Baixe o arquivo desejado:

| Artefato | Conteúdo | Disponibilidade |
|---|---|---|
| `relatorio-api` | Relatório HTML dos testes de API | Sempre |
| `relatorio-frontend` | Relatório HTML dos testes de Frontend | Sempre |
| `screenshots-api` | Screenshots de falhas — API | Apenas quando há falha |
| `screenshots-frontend` | Screenshots de falhas — Frontend | Apenas quando há falha |

> Os artefatos ficam disponíveis por **7 dias** após cada execução.
...
jobs:
  cypress-api:
    ...
    steps:
      ...
      - uses: cypress-io/github-action@v6
        with: { command: npm run cy:api }
      - uses: actions/upload-artifact@v4   # relatório (sempre)
      - uses: actions/upload-artifact@v4   # screenshots (só em falha)
  ...
  cypress-frontend:
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: cypress-io/github-action@v6
        with: { command: npm run cy:frontend }
      - uses: actions/upload-artifact@v4   # relatório (sempre)
      - uses: actions/upload-artifact@v4   # screenshots (só em falha)
```
